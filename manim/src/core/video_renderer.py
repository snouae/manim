...
class VideoRenderer:
    ...
    def combine_videos(self, topic: str):
        ...
        if not os.path.exists(output_dir):
            print(f"Output directory {output_dir} does not exist. Cannot combine videos.")
            return False
            
        search_path = os.path.join(self.output_dir, file_prefix)
        output_video_path = os.path.join(self.output_dir, file_prefix, f"{file_prefix}_final.mp4")
        output_srt_path = os.path.join(self.output_dir, file_prefix, f"{file_prefix}_final.srt")
        ...
        # Try to extract scene outline with regex
        scene_outline_match = re.search(r'(<SCENE_OUTLINE>.*?</SCENE_OUTLINE>)', plan, re.DOTALL)
        if scene_outline_match is None:
            print(f"Warning: Scene outline file does not contain the expected format. Cannot determine scene count.")
            print("This may be because the model didn't generate a proper scene outline.")
            return False
        ...
        if scene_count == 0:
            print("No scenes found in the outline. Nothing to combine.")
            return False
        ...
        if not scene_folders:
            print("No scene folders found. Nothing to combine.")
            return False
