...
class LiteLLMWrapper:
    ...
    def __call__(self, messages: List[Dict[str, Any]], metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        for msg in messages:
            if msg["type"] == "text":
                ...
                if "gemini" in self.model_name:
                    ...
                    # Add specific support for deepseek models
                    if msg["type"] == "image":
                        formatted_messages.append({
                            "role": "user",
                            "content": [
                        ...
                        raise ValueError("For Deepseek Vision, only text and image inferencing are supported")
                else:
        ...
        try:
            ...
            # Add special handling for Deepseek models
            elif "deepseek" in self.model_name:
                ...
                print(f"Using Deepseek model: {self.model_name} with API key: {api_key[:5]}...")
                ...
                response = completion(
                    ...
                    temperature=self.temperature,
                    api_key=api_key,
                    api_base="https://api.deepseek.com/v1/chat/completions",  # Explicitly set API base
                    metadata=metadata,
                    max_retries=99
                )
            else:
